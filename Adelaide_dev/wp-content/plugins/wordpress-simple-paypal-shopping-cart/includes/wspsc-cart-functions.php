...
foreach ($_SESSION['simpleCart'] as $item) {
		...
		$splitds = explode('###ADE-', $item_name);
		$return_url = add_query_arg('pageid', $splitds[1], $return_url); 
    ...
    $output .= '<div class="shopping_cart container">';
    ...
    if ($_SESSION['simpleCart'] && is_array($_SESSION['simpleCart'])) {
        ...
        <th colspan="2" class="wspsc_cart_item_name_th">' . (__("Item Name", "wordpress-simple-paypal-shopping-cart")) . '</th>' . '<th colspan="2" class="wspsc_cart_price_th">' . (__("Price", "wordpress-simple-paypal-shopping-cart")) . '</th>
        ...
        foreach ($_SESSION['simpleCart'] as $item) {
            ...
            $output .= '<tr class="wspsc_cart_item_thumb"><td colspan="2" style="overflow: hidden;">';
			...
			$itm_name = $item['name'];
			$itm_name = explode('###ADE-', $itm_name);
            $item_info = apply_filters('wspsc_cart_item_name', $itm_name[0], $item);
            ...
            $output .= "<td style='text-align: center; display: none'><form method=\"post\"  action=\"\" name='pcquantity' style='display: inline'>".wp_nonce_field('wspsc_cquantity', '_wpnonce', true, false)."
			...
			$send_item_name = $item['name'];
			$send_item_name = explode('###ADE-', $send_item_name);

            $form .= "

	            <input type=\"hidden\" name=\"item_name_$count\" value=\"" . $send_item_name[0] . "\" />
    ...
    if ($count) {
        ...
        $output .= "<tr class='wspsc_cart_total'><td colspan='2' style='font-weight: bold; text-align: right;'>" . (__("Total", "wordpress-simple-paypal-shopping-cart")) . ": </td><td colspan='2' style='text-align: center'>" . print_payment_currency(($total + $postage_cost), $paypal_symbol, $decimal) . "</td></tr>";
        ...
        if ($count){
            ...
            $output .= '<input type="image" src="' . apply_filters('wspsc_cart_checkout_button_image_src', $checkout_button_img_src) . '" name="submit" style="width:200px !important" class="wp_cart_checkout_button" alt="' . (__("Make payments with PayPal - it\'s fast, free and secure!", "wordpress-simple-paypal-shopping-cart")) . '" />';
     ...
     * New code added
     * */
    $output .= "<div class='container'><div class='cart_account'>You Can Make the Money Transfer Also.<br/>Account Details:<br/>
	BSB 065000<br/>
	Account No 11231542<br/>
    In case of money transfer you don't have to pay the card changes. <br/>You will have to contact in this mail id for further process.<br/>
    </div></div>";
