...
function _print_pdf($html) {
  ...
  if (isset($wp_query->query_vars['pdf'])) {
    ...
    $cached = PDF_CACHE_DIRECTORY . get_the_ID() . '-INVOICE.pdf';
    //$cachedmoney = PDF_CACHE_DIRECTORY . get_the_ID() . '-MONEYRECIVE.pdf';
...
/*$dompdfmony->setOptions($options);
      $htmlmony = apply_filters('pdf_html_to_dompdf', $html);

      $dompdfmony->loadHtml($htmlmony);
      //$dompdf->setBasePath(get_stylesheet_directory_uri());
      $dompdfmony->render();

      file_put_contents($cachedmoney, $dompdfmony->output());*/
    }
