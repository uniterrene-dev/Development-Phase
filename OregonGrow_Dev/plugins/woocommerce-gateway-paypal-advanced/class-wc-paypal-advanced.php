  ...
  * @return result code of the inquiry transaction
    ...
    private function inquiry_transaction($order,$order_id) {
        ...
        if($inquiry_result_arr['RESULT'] == 0 && $inquiry_result_arr['RESPMSG']=='Approved') {
            ...
            // Add order note
            $order->add_order_note( sprintf( __('Received result of Inquiry Transaction for the  (Order: %s) and with error:%s', 'wc_paypaladv' ), $order->get_order_number(), $inquiry_result_arr['RESPMSG']) );
            return 'Error';
    ...
    private function success_handler($order,$order_id,$silent_post) {
        ...
        // Add order note
		$order->add_order_note( sprintf( __('PayPal Advanced payment completed (Order: %s). Transaction number/ID: %s.', 'wc_paypaladv' ), $order->get_order_number(),$_POST['PNREF'] ) );
        ...
        if ($inq_result == 'Approved') {//if approved
            ...
            WC()->cart->empty_cart();
             ...
             // Add order note
            $order->add_order_note( sprintf( __('Payment completed for the  (Order: %s)', 'wc_paypaladv' ), $order->get_order_number() ) );

            //log the completeion
            if( $this->debug == 'yes' )
                $this->log->add( 'paypal_advanced', sprintf( __('Payment completed for the  (Order: %s)', 'wc_paypaladv' ), $order->get_order_number() ));
    ...
    private function decline_handler($order,$order_id,$silent_post) {
        ...
        // Log the status
        if ($this->debug == 'yes')
            $this->log->add( 'paypal_advanced', sprintf( __('Status has been changed to failed for order %s', 'wc_paypaladv'), $order->get_order_number()));
	...
	public function relay_response() {
		...
		if($status == 'processing' || $status == 'completed') {
         // Log
			if ($this->debug == "yes") $this->log->add( 'paypal_advanced', sprintf( __('Redirecting to Thank You Page for order %s', 'wc_paypaladv'), $order->get_order_number()));
